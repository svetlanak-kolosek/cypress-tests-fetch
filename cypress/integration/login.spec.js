...
it("Visit Login page", ()=>{
    ...
    cy.get(Locators.Login.Login).eq(0).click()
...
it.only("Login with valid data", ()=>{
    ...
    cy.get(Locators.Login.Message).should("be.visible").and("include.text", "Signed in successfully")
...
it("N-Login without email", ()=>{
    ...
    cy.get(Locators.Login.Message).should("be.visible").and("include.text", "Invalid Email")
})
...
it("N-Login whit invalid email, using email without @", ()=>{
    ...
    cy.get(Locators.Login.Email).eq(0).then(($input)=>{
        expect($input[0].validationMessage).to.eq("Please include an '@' in the email address. 'kolosekmailinator.com' is missing an '@'.")
    })
})
...
it("N-Login without password", ()=>{
    ...
    cy.get(Locators.Login.Email).type("kolosek@mailinator.com")
    cy.get(Locators.Login.SignIn).click()
    cy.get(Locators.Login.Message).should("be.visible").and("include.text", "Invalid Email or password")
})
...
it("N-Login with invalid password, using less than 6 characters", ()=>{
    cy.visit("https://notify.kolosek.com/")
    cy.get(Locators.Login.Login).eq(0).click()
    cy.get(Locators.Login.SignIn).should("be.visible").and("have.value", "Sign in")
    cy.get(Locators.Login.Email).type("kolosek@mailinator.com")
